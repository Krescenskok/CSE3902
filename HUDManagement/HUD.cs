...
namespace Sprint5
{
    public class HUD
    {
        ...
        {
            heartManager.ChangeDifficulty(hp, max);
        }
        ...
        public void DrawTop(SpriteBatch spriteBatch)
        {
            ...
            if (storage.BSlotItems.ContainsKey(storage.BSlot))
            {
                storage.BSlotItems[storage.BSlot].Draw(spriteBatch);
            }
            storage.ASlotItems[storage.ASlot].Draw(spriteBatch);
            spriteBatch.DrawString(HUDfont, "X" + LinkInventory.Instance.RupeeCount, storage.RupeeCountLocation, Color.White);
            spriteBatch.DrawString(HUDfont, "X" + LinkInventory.Instance.KeyCount, storage.KeyCountLocation, Color.White);
            spriteBatch.DrawString(HUDfont, "X" + LinkInventory.Instance.BombCount, storage.BombCountLocation, Color.White);
            foreach (IItems heart in storage.DrawnHearts)
            {
